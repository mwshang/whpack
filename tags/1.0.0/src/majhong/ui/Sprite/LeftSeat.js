 ...
 */
var LeftSeat = DeskSeat.extend({
    ctor: function (data) {
        this._super(data, 'leftseat');
        this.root = ccs.load("res/MajhongBase/MajhongLefterPanel.json").node;
  ...
  addCardOut:function(cardObj)
  {
      ...
      var card = new CardLeftDesk(cardObj,floor);
        ...
        if(this.uid == data['uid']) {
            ...
            for (var i = 0; i < cards.length; i++) {
                ...
                for (var j = 0; j < pais.length; j++) {
                    var card = new CardLeftShow(pais[j]);
                    card.y = startPos - h * j;
                    card.x = 0;
    ...
    setHandCardsBack :function()
    {
        ...
        for (var i =0;i<length;i++)
        {
            ...
            {
                card = new CardLeftStand();
            }else
            {
                var Date={};
                Date['type'] ='H';
                Date['value'] = 1;
                card = new CardLeftShow(Date);
                card.setRotationX(-5);
                card.SetBack();
            }
            this.panel_cardIn.addChild(card,i);
    ...
    addGangCards: function (data) {
      ...
      {
        var key = cards[i];
        var outCard = new CardLeftShow(key);
    ...
    addBuzhangCardsPanel: function (cardObj,gang_type,sourceChair) {
        ...
        for(var i = 3 ; i >= 0; i--)
        {
            var card = new CardLeftShow(cardObj);
            if(i == 3)
            {
    ...
    addChiCardsPanel: function (cards) {
        ...
        for(var i = 2 ; i >= 0 ; i--)
        {
            var card = new CardLeftShow(cards[i]);
            card.setScale(1-(2-i)*0.03);
            card.setPosition(this.pengPanel.getChildByName("node"+i).getPosition());
            if(i == 1)
            {
    ...
    showHandInCards: function () {
        ...
        {
            var card = new CardLeftShow(this.cardInList[i]);
            card.x  = 0;
    ...
    addHu: function (msg) {
      ...
      {
        var cardShow = new CardLeftShow(handCards[i],CARD_SITE.RECORD);
      ...
      {
        var cardShow = new CardLeftShow(huHands[i],CARD_SITE.RECORD);
        var length = this.cardInArray.length;
  ...
  playRecordMoCard: function (data) {
    var card = new  CardLeftShow(data,CARD_SITE.RECORD);
    var length = this.cardInArray.length;
    card.setScale(this.cardInArray[length-1].getScale()-0.05);
  ...
  playRecordSendAction:function(card)
  {
    ...
    runRecordBuhuaOpAction: function (data) {
        ...
        for (var i = 0; i < huapai.length; i++) {
            var card = new CardLeftShow(huapai[i],CARD_SITE.RECORD);
            if(MajhongInfo.MajhongNumber > 14)
            {
    ...
    this.panel_cardIn.removeAllChildren();
      ...
      for (var p in cards) {
          ...
          for (var i = 0; i < arr.length; i++) {
              var card = new CardLeftShow(arr[i],CARD_SITE.RECORD);
  ...
  recordAddGangPanel: function (cardObj) {
    ...
    for(var i = 0 ; i < 4;i++)
    {
      var card = new CardLeftShow(cardObj,CARD_SITE.RECORD);
      card.x  = 0;
