...
SSSPoker.PokerPai = cc.Class.extend({
    ctor: function (pai) {
        this.object = {};
        if (typeof(pai) == 'string') {
            this.type = parseInt(pai[0]);
            this.value = parseInt(pai.substr(0,pai.length));
            this.key = pai;
        ...
        this.num = SSSPoker.PokerPaiCard[this.key];
    },
    ...
    imageOfPai:function(){
        return "YJ"+SSSPoker.PokerPaiImage.ImgOfCard(this.key);
    },

    // 获取花色image
    colorImageOfPai:function(){
        return SSSPoker.PokerPaiImage.ImgOfColor(this.object);
    },

    // 获取点数图片
    pointImageOfPai:function(){
        return SSSPoker.PokerPaiImage.ImgOfPoint(this.object);
    }
});
