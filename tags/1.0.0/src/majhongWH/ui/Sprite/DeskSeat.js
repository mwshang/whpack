 ...
 */
var WHMJDeskSeat = cc.Layer.extend({
  root: null,
  panel_root: null,
  panel_cardIn: null,
  panel_cardOut: null,
  ...
  isAlreadyTing: 0,
  gap_card: 0,
  isbubai:1,          //皮子杠的时候是否补牌
  cardGDGangArry:null,
  cardGdGangPanel:null,

  ctor: function (data, info) {
    this._super();
    this.info = info;
    this.uid = data["uid"];
    JJLog.print('初始化！！！！  ==='+ JSON.stringify(data));
    this.isbubai = data["gdGangBuPai"];
  ...
  initUI: function () {
    ...
    if (MajhongInfo.GameMode == GameMode.RECORD) {
      ...
      btn_gang.setTag(OPERATIONTYPE.BUZHANG);
      ...
      btn_bu.setTag(OPERATIONTYPE.TING);
    ...
    this.cardGdGangPanel = ccui.helper.seekWidgetByName(this.panel_root, "panel_GDgangpanel");
  },
  ...
  onSyncDelCards: function (data) {
     JJLog.print("DeskSeat onSyncDelCards"+JSON.stringify(data));
    ...
    if (this.checkMsg(data)) {
      var cardId = data['msg']['type'] +data['msg']['value'];
      if(cardId == whmajhong.table.JinPaiId || cardId == whmajhong.table.PiziId1||cardId == whmajhong.table.PiziId2||cardId == whmajhong.table.PiziId3)
      {
    ...
    if (data["uid"] == this.uid) {
      ...
      switch (opType) {
        ...
        case OPERATIONNAME.BUZHANG:
        {
          ...
          this.resetPanelInChild();
          this.addGDGangCardsPanel(opCard,msg["origin"]);
        }
          break;
  ...
  playTipAnim: function (eType) {
    var tip = new OpTip(eType);
    tip.setPosition(0, 0);
    this.node_tip.addChild(tip, 1000);
  },
  ...
  addGDGangCardsPanel:function(cardObj)
  {
    //todo
  },
  ...
  removeBuzhangCards: function (data) {
    ...
    switch (gang_type) {
      ...
      case OPER_GANG_TYPE.GANG_MING:
      {
        ...
        break;
      case OPER_GANG_TYPE.Gang_Pizi:
      {
        this.removeHandInCard(1);
      }
        break;
      case OPER_GANG_TYPE.Gang_Laizi:
      {
        this.removeHandInCard(1);
      }
        break;
  ...
  initLastCards: function () {
    ...
    //var cardOuts = ["T2","T9","T9","T9","T9","T9","T9","T9","T9","T9"];
    //var cardOuts = ["T3","T3","T3","T3","T3","T3","T3","T3","T3","T3","T3","T3","T3","T3","T3","T3","T3","T3"];
    ...
    for (var k = 0; k < funcArr.length; k++) {
      ...
      switch (funcType) {
        case OPERATIONNAME.GANG:
        {
          ...
          this.addGDGangCardsPanel(cardObj,funcInfo["origin"]);
        }
          break;
        case OPERATIONNAME.BUZHANG:
        {
    ...
    var isTing = info['isTing'];
    this.isAlreadyTing = isTing;
    if(isTing > 1)
    {
      this.setHandCardsBack();
    }
  ...
  if(info['ting'] == null || info['ting'] == undefined)
    {
      info['ting'] = [];
    }
    this.panel_replay.getChildByTag(OPERATIONTYPE.TING).setEnabled(info['ting'].length > 0 ? true : false);
    this.panel_replay.getChildByTag(OPERATIONTYPE.TING).setBright(info['ting'].length > 0 ? true : false);
    //this.panel_replay.getChildByTag(OPERATIONTYPE.TING).setEnabled(info['bu'].length > 0 ? true : false);
    //this.panel_replay.getChildByTag(OPERATIONTYPE.TING).setBright(info['bu'].length > 0 ? true : false);
...
//    btn.runAction(seq);
  ...
  recordBuzhangResp: function (data) {
    JJLog.print('回放杠=' + JSON.stringify(data));
    var opCard = data['opCard'];
    this.addBuzhangCardsPanel(opCard,data['origin']);
    this.addGDGangCardsPanel(opCard,data["origin"]);
  ...
  recordRemoveHandinBuzhangCards:function(data)
  {
    ...
    switch (gang_type) {
      ...
      case OPER_GANG_TYPE.Gang_Pizi:
      {
        this.recordRemoveHandCard(opCard,false);
      }
        break;
      case OPER_GANG_TYPE.Gang_Laizi:
      {
