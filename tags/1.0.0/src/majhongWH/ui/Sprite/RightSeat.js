 ...
 */
var WHMJRightSeat = WHMJDeskSeat.extend({
    ctor: function (data) {
      this._super(data, 'rightseat');
      this.root = ccs.load("res/MajhongWH/WuHanRighterPanel.json").node;
  ...
  addCardOut:function(cardObj)
  {
      ...
      var card = new WHCardRightDesk(cardObj,floor);
        ...
        if(this.uid == data['uid']) {
            ...
            for (var i = 0; i < cards.length; i++) {
                ...
                for (var j = 0; j < pais.length; j++) {
                    var card = new WHCardRightShow(pais[j]);
                    card.y = startPos + h * j;
                    card.x = 0;
    ...
    setHandCardsBack :function()
    {
        ...
        for (var i =0;i<length;i++)
        {
            ...
            {
                card = new WHCardRightStand();
            }else
            {
                var Date={};
                Date['type'] ='H';
                Date['value'] = 1;
                card = new WHCardRightShow(Date);
                card.setRotationX(5);
                card.SetBack();
            }
    ...
    addMoCard: function () {
        this._super();
        var length = this.cardInArray.length;
        var card = new WHCardRightStand(this);
    ...
    addHideInCard: function () {

      var cardInSize = this.panel_cardIn.getContentSize();
        var card = new WHCardRightStand();
        var cardSize = card.getContentSize();
        var posY = 0;
        card.setPosition(cardInSize.width*0.5,
          posY  + this.gap_stand*i);
        this.panel_cardIn.addChild(card,20-i);
        this.cardInArray.push(card);
    ...
    addGangCards: function (data) {
      ...
      {
        var key = cards[i];
        var outCard = new WHCardRightShow(key);
    ...
    addBuzhangCardsPanel: function (cardObj,gang_type,sourceChair) {
        ...
        for(var i = 0 ; i < 4;i++)
        {
            var card = new WHCardRightShow(cardObj);
            if(i == 3)
            {
    ...
    addGDGangCardsPanel:function(cardObj,gang_type)
    {
        if(gang_type> 3)
        {
            ...
            var index = length%WHCommonParam.DeskOneNum;
            var floor = Math.floor(length/WHCommonParam.DeskOneNum);
            JJLog.print("FFFFFFFFFFFFF="+ floor);
            var card = new WHCardRightDesk(cardObj,floor);
            var height = card.getContentSize().height;
            var off = 0;
            var y = (length-length*(length-1)/2*0.015)*height*0.55;
            var x = y/-2.5;
            card.setScale(1-0.015*length);
            card.x = x+15;
            card.y = y+10;
            this.cardGdGangPanel.addChild(card,40-length+off*50);
    ...
    addChiCardsPanel: function (cards) {
        ...
        for(var i = 0 ; i < cards.length;i++)
        {
            var card = new WHCardRightShow(cards[i]);
            card.setScale(1-i*0.03);
            card.setPosition(this.pengPanel.getChildByName("node"+i).getPosition());
            if(i == 1)
            {
    ...
    showHandInCards: function () {
        ...
        {
            var card = new WHCardRightShow(this.cardInList[i]);
            card.x  = 0;
    ...
    addHu: function (msg) {
      ...
      {
        var cardShow = new WHCardRightShow(handCards[i],CARD_SITE.HAND_HU);
      ...
      {
        var cardShow = new WHCardRightShow(huHands[i],CARD_SITE.HAND_HU);
        var length = this.cardInArray.length;
    ...
    runRecordBuhuaOpAction: function (data) {
        ...
        for (var i = 0; i < huapai.length; i++) {
            var card = new WHCardRightShow(huapai[i]);
            if(MajhongInfo.MajhongNumber > 14)
            {
    ...
    initRecordHandCards:function() {
      var cards = whmajhong.record.rightHandCards;
      this.panel_cardIn.removeAllChildren();
        var allCards = new Array();
        for (var p in cards) {
            var arr = cards[p];
            for (var i = 0; i < arr.length; i++) {
                var card = new WHCardRightShow(arr[i]);
  ...
  playRecordMoCard: function (data) {
    var card = new WHCardRightShow(data);
    var length = this.cardInArray.length;
    card.setScale(this.cardInArray[length-1].getScale()-0.05);
  ...
  playRecordSendAction:function(card)
  {
    ...
    outCard.runAction(cc.sequence(
      spawnShow,cc.delayTime(WHCommonParam.ShowDelayTime),
      cc.callFunc(function () {
        this.removeFromParent();
      }.bind(outCard))));
