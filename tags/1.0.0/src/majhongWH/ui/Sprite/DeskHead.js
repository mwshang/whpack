 ...
 */
var WuHanDeskHead  = cc.Layer.extend({
    playerData:null,
    text_name:null,
    text_score:null,
    panel_root:null,
  ...
  sprite_msg:null,
    ...
    ctor:function(data){
        ...
        this.uid = data["uid"];
        var root = ccs.load("res/MajhongWH/WuHanHead.json").node;
        this.addChild(root);
        ...
        this.text_score.setString(this.getScoreStr(parseFloat(this.playerData["coinNum"])));
        this.text_name = ccui.helper.seekWidgetByName(root,"text_name");
        //游戏中名字显示 截断
        this.text_name.setString(base64.decode(this.playerData["nickName"]).substring(0,6));
        this.sprite_head = ccui.helper.seekWidgetByName(root,"sprite_head");
        // TODO 
        // this.text_name.setString(this.playerData["uid"]);
        ...
        this.img_maizhuang = ccui.helper.seekWidgetByName(root,"img_maizhuang");
        this.img_maizhuang.setVisible(false);
    },
    ...
    onEnter: function () {
        ...
        if(pos.x < size.width*0.5)
        {
          ...
          this.img_maizhuang.x = size.width+1;
    ...
    getScoreStr:function(score) {
        if (score) {            
            score = score * basecashdata.curBaseCash;
    ...
    registerAllEvents: function () {
      ...
      qp.event.listen(this, 'mjMaiZhuang', this.onSynmaizhuang.bind(this));
      qp.event.listen(this, 'mjCaiTou', this.onSyncZhenQiJiaBa);
    },

    onSyncZhenQiJiaBa:function(data)
    {
        for(var i in  data['players'])
        {
            if(this.uid == i)
            {
                var curr = parseInt(this.text_score.getString()) + this.getScoreStr(data['players'][i]) ;
                this.text_score.setString(curr);
                break;
            }
        }

    },
    onSynmaizhuang:function(data)  //显示是否买庄
    {
        if(data['uid'] == this.uid)
        {
            if(data['op']== 3 || data['op'] == 4)
            {
                this.img_maizhuang.setVisible(true);
            }else
            {
                this.img_maizhuang.setVisible(false);
            }
    ...
    removeAllEvents: function () {
      ...
      qp.event.stop(this, 'mjMaiZhuang');
      qp.event.stop(this, 'mjCaiTou');
    },
  ...
  onGameResult: function (data) {
    ...
    for(var i = 0; i < players.length;i++)
    {
       var info = players[i];
        ...
        {
          this.text_score.setString(this.getScoreStr(parseFloat(info["coinNum"])));
          break;
        }
    }
  },
