 ...
 */
var WHMJRecordControll = cc.Layer.extend({
    panel_control:null,
    btn_play:null,
    btn_pause:null,
    btn_backward:null,
    ...
    ctor: function () {
      this._super();
      var root = ccs.load("res/MajhongBase/MajhongRecordControll.json").node;
      this.addChild(root);
      ...
      this.btn_exit.addClickEventListener(function () {
        ...
        {
            var recordId = whmajhong.record.recordId;
            JJLog.print("分享="+ recordId +"wanjian="+ base64.decode(hall.user.nickName));
            hall.net.wxShareURL('武汉麻将','玩家['+base64.decode(hall.user.nickName) + ']分享了一个回访码:'+recordId+'在大厅点击进入战绩页面然后点击查看他人回放输入回访码.', 0);
    ...
    {
      this.text_process.setString('进度:'+ whmajhong.record.stepNow+'/'+whmajhong.record.stepAll);
    },

    onPlay: function () {
      this.btn_play.setVisible(false);
      this.btn_pause.setVisible(true);
      whmajhong.record.setPlayStatus(RecordStatus.PLAY);
    },

    onPause: function () {
      this.btn_play.setVisible(true);
      this.btn_pause.setVisible(false);
      whmajhong.record.setPlayStatus(RecordStatus.PAUSE);
    },
