var PDKChat = cc.Layer.extend({
    ...
    ctor:function(){
        ...
        if(GAMENAME =='qidong')
        {
            JsonUsualItem = QDMajhongJson.ChatUsualItem;
        }
        ...
        for (var i = 0; i <  hall.getPlayingGame().CHAT_USUALMSG.length; ++i) {
            this.list_usual.pushBackDefaultItem();
            var item = this.list_usual.getItem(i);
            item.getChildByName("Text_Content").setString(hall.getPlayingGame().CHAT_USUALMSG[i]);
        }
    },
