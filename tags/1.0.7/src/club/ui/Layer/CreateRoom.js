...
var clubCreateRoom = cc.Layer.extend({
  gameBtns:null,
  gameIndex:0,
  gameShowPanels:null,
  //武汉麻将
...
//罗松
  ...
  ctor: function (data, isAuto, cb) {
    ...
    if (true) {//底金UI初始化
        ...
        this.baseScoreTxt.setFontSize(this.baseScoreTxt.getFontSize()-1)
        this.baseScoreTxt.setPositionY(this.baseScoreTxt.getPositionY() + 1)
   ...
   // 十三水
    ...
    this.btn_create.addClickEventListener(function () {
      if (this.isAuto) {
...
onClickNNRound:function (sender) {
  ...
  onCreateRoom:function()
  {
    ...
    roomData["uid"] = hall.user.uid;
      JJLog.print('俱乐部创建房间=');
      console.log("俱乐部创建房间=====",this.gameIndex);
    if(this.gameIndex == 0)
    {
      ...
      roomData['serverType'] = 'wuhan';

      var roomConfig = JSON.stringify(roomData);
            club.net.memberCreateTable(this.pid, "武汉麻将", roomConfig, 'wuhan', function (data) {
                console.log("data", data);
                if (data["code"] == 200) {
                    club.gameRoomId = data.data.tableId;
                    hall.joinPrivate(whmajhong.appId, data.data.tableId, function (data) {
                        if (data["code"] == 200) {
    ...
    else if(this.gameIndex == 1)  // 十三水
    {
        ...
        JJLog.print('十三水创建==');
        var roomConfig = JSON.stringify(roomData);
        club.net.memberCreateTable(this.pid, "十三水", roomConfig, 'shisanshui', function (data) {
            console.log("data", data);
            if (data["code"] == 200) {
                club.gameRoomId = data.data.tableId;
                hall.joinPrivate(SSSPoker.appId, data.data.tableId, function (data) {
                    if (data["code"] == 200) {
  ...
  onCreateAutoRoom:function()
  {
    ...
    if(this.gameIndex == 0)
    {
      roomData['rounds'] = this.whOpArray[0] ==1?8:16;
      roomData['aaGem'] = this.whOpArray[2]==1?0:1;
      roomData['yuanLaiFan'] = this.whOpArray1[0];
      roomData['fengLaiFan'] = this.whOpArray1[1];
      roomData['yiJiuLaiFan'] = this.whOpArray1[2];
      roomData['lianJinFan'] = this.whOpArray1[3];
      roomData['tableName'] = 'WHMajhong';
      roomData['serverType'] = 'wuhan';
      //2018.12.20 mwshang增加底金
      roomData['baseScore'] = basecashdata.curBaseCash;//底金
      ...
      var roomConfig = JSON.stringify(roomData);
      club.net.createAutoTable(this.pid, "武汉麻将", roomConfig, 'wuhan', function (data) {
        console.log("data", data);
        if (data["code"] == 200) {
          cb();
          var dialog = new JJConfirmDialog();
    ...
    else if(this.gameIndex == 1)  // 十三水
    {
        ...
        roomData['mode'] = this.item_ops["moshi"]         //模式
        roomData['isQiangZhi'] = 1;                       //强制比牌
        roomData['serverType'] = 'shisanshui';
        ...
        JJLog.print('十三水创建==');
        var roomConfig = JSON.stringify(roomData);
        club.net.createAutoTable(this.pid, "十三水", roomConfig, 'shisanshui', function (data) {
            console.log("data", data);
            if (data["code"] == 200) {
                cb();
                var dialog = new JJConfirmDialog();
    ...
    else if(this.gameIndex == 2)
    {
      ...
      roomData["shangZhuang"] = 0;    //上庄分数
      roomData["tuiZhu"] = 0; //闲家推注
      roomData['serverType'] = 'douniu';
      
      var roomConfig = JSON.stringify(roomData);
        club.net.createAutoTable(this.pid, "牛牛", roomConfig, 'douniu', function (data) {
            console.log("data", data);
            if (data["code"] == 200) {
                cb();
                var dialog = new JJConfirmDialog();
